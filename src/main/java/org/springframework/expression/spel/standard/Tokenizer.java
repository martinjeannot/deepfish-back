...
/**
 * Redefinition of Spring's exact same class with :
 * - added support of some extended ASCII characters (needed for talents search)
 */
public class Tokenizer {
  ...
  private static final byte IS_EXTENDED = 0x05;
  ...
  static {
    ...
    for (int ch = 0xc0; ch <= 0xff; ch++) { // from 192 to 255
      FLAGS[ch] |= IS_EXTENDED;
    }
  }
        ...
        switch (ch) {
          ...
          default:
            throw new IllegalStateException(
                "Cannot handle (" + Integer.valueOf(ch) + ") '" + ch + "'");
        }
      }
    }
    return this.tokens;
  ...
  private void lexQuotedStringLiteral() {
    ...
    this.pos++;
    this.tokens
        .add(new Token(TokenKind.LITERAL_STRING, subarray(start, this.pos), start, this.pos));
  }
  ...
  private void pushHexIntToken(char[] data, boolean isLong, int start, int end) {
    if (data.length == 0) {
      if (isLong) {
        raiseParseException(start, SpelMessage.NOT_A_LONG,
            this.expressionString.substring(start, end + 1));
      } else {
        raiseParseException(start, SpelMessage.NOT_AN_INTEGER,
